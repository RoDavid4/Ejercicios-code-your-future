...
public class AutorService {
  ...
  public void eliminarAutor(UUID id) throws MyException {
    ...
    Optional<Autor> respuesta = autorRepository.findById(id);
    if (respuesta.isPresent() && respuesta.get().getAutorActivo()) {
      Autor autor = respuesta.get();
      autor.setAutorActivo(false); // seteamos el autor como INACTIVO
    } else {
      throw new MyException("El autor con el ID especificado no existe o esta inactivo.");
    }
  }

  @Transactional
  public void reactivarAutor(UUID id) throws MyException {
    validarId(id);
    Optional<Autor> respuesta = autorRepository.findById(id);
    if (respuesta.isPresent() && !respuesta.get().getAutorActivo()) {
      Autor autor = respuesta.get();
      autor.setAutorActivo(true); // seteamos el autor como ACTIVO
    } else {
      throw new MyException("El autor con el ID especificado no existe o ya está activo.");
    }
  }
